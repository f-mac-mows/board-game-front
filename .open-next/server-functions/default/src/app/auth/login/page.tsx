...
export default function LoginPage() {
    ...
    const handleGoogleLogin = () => {
        ...
        window.location.href = `https://api.walrung.com/oauth2/authorization/google`;
    };
        ...
        <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
            <div className="max-w-md w-full space-y-8 bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-xl">
                <div className="text-center">
                    ...
                    <p className="mt-2 text-sm text-slate-400">보드게임에 오신 것을 환영합니다.</p>
                </div>
                
                <form className="mt-8 space-y-6" onSubmit={handleLogin}>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text=slate-300">이메일</label>
                            <input
                                type="email"
                                required
                                className="mt-1 block w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all"
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full py-3 px-4 bg-blue-600 hober:bg-blue-700 disabled:bg-blue-800 text-white font-bold rounded-lg transition-colors shadow-lg shadowblue-500/20"
                    >
                        {isLoading ? "로그인 중..." : "로그인"}
                    </button>
                </form>
